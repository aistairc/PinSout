...
def merge_dup_plane2(plane_list, normal_vector):
    print "merge_dup_plane"
    print
    new_plane_list = list()
    new_normal_vector = list()
    new_bbox_list = list()
    prev_bbox = []
    duple_index = [[i] for i in range(len(plane_list))]
    for p_i in plane_list:
        prev_bbox.append(get_range(p_i, 0.4))
    dup_plane_index = list()
    for i in range(len(plane_list) - 1):
        for j in range(i + 1, len(plane_list)):
            if check_bbox(prev_bbox[i], prev_bbox[j]):
                model_cos = np.dot(
                ...
                if math.fabs(round(model_cos, 1)) == 1.0:
                    ...
                    if distance_bw_planes <= 0.2:
                        duple_index[i].append(j)
        print
    print duple_index
    dup_list2 = []
    for d_i in duple_index:
        if len(d_i) > 1:
            dup_list2.append(d_i)
    remove_list = []
    if len(dup_list2) != 0:
        dup_list2 = sorted(dup_list2, key=cmp_to_key(lenSort))
        count = 0
        while True:
            temp = dup_list2[0]
            checkvalue = []
            used_index = []
            for j in range(len(dup_list2)):
                for j_i in dup_list2[j]:
                    if j_i in temp:
                        for j_j in dup_list2[j]:
                            if j_j not in temp:
                                checkvalue.append(j_j)
                        used_index.append(j)
                        break
            count += 1

            if len(checkvalue) != 0:
                temp.extend(checkvalue)
            remove_list.append(temp)
            if len(used_index) == len(dup_list2):
                break
            if len(used_index) != 0:
                temp_list = []
                for d_i in range(len(dup_list2)):
                    if d_i not in used_index:
                        temp_list.append(dup_list2[d_i])
                if len(temp_list) != 0:
                    dup_list2 = temp_list
        for r_i in remove_list:
            temp_cloud = pcl.PointCloud()
            temp_cloud_list = []
            for r_v in r_i:
                temp_cloud_list.extend(plane_list[r_v].to_list())
            temp_cloud.from_list(temp_cloud_list)
            new_plane_list.append(temp_cloud)
            new_normal_vector.append(normal_vector[r_i[0]])
            new_bbox_list.append(get_range(temp_cloud, 0.4))
        all_list = reduce(lambda x, y: x+y, remove_list)
        for p_i in range(len(plane_list)):
            if p_i not in all_list:
                new_plane_list.append(plane_list[p_i])
                new_normal_vector.append(normal_vector[p_i])
                new_bbox_list.append(get_range(plane_list[p_i], 0.4))
...
def merge_dup_plane(plane_list, normal_vector):
            ...
            if check_bbox(main_bbox, sub_bbox):
                model_cos = np.dot(
                    ...
                    [normal_vector[i][0], normal_vector[i][1], normal_vector[i][2]])
                               * np.linalg.norm(
                            [normal_vector[j][0], normal_vector[j][1], normal_vector[j][2]]))
                print math.fabs(round(model_cos, 1)), i, j
                if math.fabs(round(model_cos, 1)) >= 0.95:
    ...
    if len(dup_plane_index) != 0:
        ...
        test= []
        for i in new_plane_list:
            test.append(get_range(i, 0.4))
        # return new_plane_list, new_normal_vector, new_bbox_list
        return new_plane_list, new_normal_vector, test
    else:
        test = []
        for i in new_plane_list:
            test.append(get_range(i, 0.4))
        # return plane_list, normal_vector, prev_bbox
        return plane_list, normal_vector, test
...
def get_plane_list(clustered_cloud):
    ...
    while True:
        ...
        if math.fabs(model_cos) < 0.8:
            ...
            cloud = outliers_p
    # merge_dup_plane2(plane_list, normal_vector)
    new_plane_list, new_normal_vector, new_bbox_list = merge_dup_plane2(plane_list, normal_vector)
...
def make_wall_info(cloud):
    ...
    for clustered_cloud in cluster_list:
        ...
        # for ei in range(len(wall_point_list)):
        #     out_filename3 = "/home/dprt/Desktop/pcd/" + "_" + str(ei) + ".pcd"
        #     pcl.save(wall_point_list[ei], out_filename3)
...
if __name__ == "__main__":
    ...
    cloud = pcl.load("/home/dprt/pinsout/data/Sigspatial_LBJ/wall_1.ply")
