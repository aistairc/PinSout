...
indoor3d_data_dir = os.path.join(data_dir, 'SCscan_data3')
NUM_POINT = 4096
H5_BATCH_SIZE = 1000
...
output_dir = os.path.join('/root/pointnet/sem_seg/SCscan_data_3')
...
for i, data_label_filename in enumerate(data_label_files):
    ...
    data, label = indoor3d_util.room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=1.0, stride=0.5, random_sample=False, sample_num=None)
...
# indoor3d_data_dir = os.path.join(data_dir, '5class_dataset7')
# NUM_POINT = 4096
# H5_BATCH_SIZE = 1000
# data_dim = [NUM_POINT, 9]
# label_dim = [NUM_POINT]
...
#
# # Set paths
# filelist = os.path.join(BASE_DIR, 'meta/area1_data_label.txt')  # npy file list ex)Area_1_WC_1.npy
# data_label_files = [os.path.join(indoor3d_data_dir, line.rstrip()) for line in open(filelist)]
# output_dir = os.path.join(data_dir, '/root/pointnet/sem_seg/Area_1_5class')
# if not os.path.exists(output_dir):
#     os.mkdir(output_dir)
