...
# DATA_PATH = os.path.join(ROOT_DIR, 'data', 'npytest')
DATA_PATH = os.path.join('/home/dprt/Desktop/SC_DEMO/New Folder')
# DATA_PATH = os.path.join(ROOT_DIR, 'data', 'SC_TEST_5cls')
# DATA_PATH = os.path.join('/home/dprt/Desktop/stanford_data/sc_test_data')
...
# g_easy_view_labels = [0, 1, 2, 7, 8]
# g_easy_view_labels = [0, 1, 2, 3, 4, 5, 6]
...
def collect_point_label2(anno_path, out_filename, file_format='ply'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.ply')):  # 확장자가 txt인 파일들의 목록을 가져옴
        cls = os.path.basename(f).split('.')[0]  # 입력받은 경로의 base name(기본 이름)을 반환
        if cls not in g_classes:  # note: in some room there is 'staris' class..
            cls = 'clutter'
        # points = np.loadtxt(f, delimiter=',') # 좌표들의 구분이 comma로 구분되어 있을 경우 사용 (default= space)
        plydata = PlyData.read(f)
        points = list()
        # ply_z = plydata.elements[0].data['z']
        # ave_z = np.mean(np.array(ply_z))
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):  # 확장자가 txt인 파일들의 목록을 가져옴
        cls = os.path.basename(f).split('_')[0]  # 입력받은 경로의 base name(기본 이름)을 반환
        if cls not in g_classes:  # note: in some room there is 'staris' class..
            # cls = 'clutter'
            print cls
            continue
...
# room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=1.0, stride=0.5,
#                                                  random_sample=False, sample_num=None)
# data_label_filename : npy 파일들의 정보, num_point : 4096,
